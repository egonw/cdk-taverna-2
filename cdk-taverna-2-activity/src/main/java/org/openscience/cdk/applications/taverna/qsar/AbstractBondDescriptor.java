...
public abstract class AbstractBondDescriptor extends AbstractCDKActivity {
	...
	public String getFolderName() {
		return Constants.QSAR_BOND_DESCRIPTOR_FOLDER_NAME;
	}
		...
		try {
			for (Iterator<CMLChemFile> iter = inputList.iterator(); iter.hasNext();) {
				...
				for (IAtomContainer molecules : moleculeList) {
					try {
						for (int j = 0; j < molecules.getBondCount(); j++) {
							DescriptorValue value = descriptor.calculate(molecules.getBond(j), molecules);
							molecules.getBond(j).setProperty(value.getSpecification(), value);
						}
