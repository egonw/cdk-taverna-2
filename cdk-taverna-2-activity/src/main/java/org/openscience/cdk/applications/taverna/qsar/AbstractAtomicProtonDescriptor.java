...
public abstract class AbstractAtomicProtonDescriptor extends AbstractCDKActivity {
	...
	protected void addInputPorts() {
		addInput(AbstractAtomicProtonDescriptor.INPUT_PORT, 1, true, null, byte[].class);
	}

	@Override
	protected void addOutputPorts() {
		for (String name : AbstractAtomicProtonDescriptor.RESULT_PORTS) {
			addOutput(name, 0);
		}
	}
	...
	public String getFolderName() {
		return Constants.QSAR_ATOMIC_PROTON_DESCRIPTOR_FOLDER_NAME;
	}
	...
	protected Map<String, T2Reference> work(Map<String, T2Reference> inputs, AsynchronousActivityCallback callback)
				...
				for (IAtomContainer molecules : moleculeList) {
					try {
						for (int j = 0; j < molecules.getAtomCount(); j++) {
							...
							if (molecules.getAtom(j).getSymbol().equals("H")) {
		...
		try {
			...
			outputs.put(AbstractAtomicProtonDescriptor.RESULT_PORTS[0], containerRef);
			dataArray = new ArrayList<byte[]>();
			...
			outputs.put(AbstractAtomicProtonDescriptor.RESULT_PORTS[1], containerRef);
		} catch (Exception e) {
			e.printStackTrace();
			// TODO exception handling
		}
