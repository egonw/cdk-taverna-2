...
/**
 * Class which represents the add explicit hydrogens activity.
 * 
 * @author Andreas Truszkowski
 * 
 */
public class AddExplicitHydrogensActivity extends AbstractCDKActivity {

	public static final String ADD_EXPLICIT_HYDROGENS_ACTIVITY = "Add Explicit Hydrogens";

	/**
	 * Creates a new instance.
	 */
	public AddExplicitHydrogensActivity() {
	...
	public String getActivityName() {
		return AddExplicitHydrogensActivity.ADD_EXPLICIT_HYDROGENS_ACTIVITY;
	}

	@Override
	public String getDescription() {
		return "Descriptions: " + AddExplicitHydrogensActivity.ADD_EXPLICIT_HYDROGENS_ACTIVITY;
	}
	...
	public Map<String, T2Reference> work(Map<String, T2Reference> inputs, AsynchronousActivityCallback callback)
		...
		for (int i = 0; i < containers.length; i++) {
			try {
				Tools.addExplicitHydrogens(containers[i]);
				taggedMoleculesList.add(CMLChemFileWrapper.wrapAtomContainerInChemModel(containers[i]));
			} catch (Exception e) {
				ErrorLogger.getInstance().writeError("Error during adding explicit hydrogens!", this.getActivityName(), e);
			}
		}
		// Congfigure output
		try {
